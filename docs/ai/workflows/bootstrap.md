...
Run `python scripts/detect-env.py` explicitly before the legacy probes below.
Before any scaffold writes, run `python scripts/ai/core_sync.py --check` and
...
Optional current request arguments: `--dry-run` (sandbox preview, no side effects) and/or project slug. If slug empty, ask via the runtime's question interface.
...
- `NO_ENV_DETECT` -> Run `python scripts/detect-env.py` on the actual host and investigate failures; never fabricate a report.
- `NO_PYTHON` (only when the hook itself failed) -> Install Python 3.13+ and reopen Claude. This is the only flag that cannot be auto-diagnosed from `env-detect.json` because the file does not exist.
- `REPO_NOT_FOUND` -> `/bootstrap` (Mode A) links to a repo **you** created by hand; the active token cannot see `$OWNER/$SLUG`. Two causes: the empty repo was never created, or the fine-grained token is not scoped to it. Remedy: (1) create the EMPTY repo at https://github.com/new (no README/.gitignore/license); (2) mint a fine-grained token via the template URL in the GitHub-access preflight (Resource owner = your login; Repository access -> Only select repositories -> `$OWNER/$SLUG`; permissions Contents / Pull requests / Workflows / Administration = Read and write); (3) add the token to `.env` as `GITHUB_PERSONAL_ACCESS_TOKEN=github_pat_...` (parsed literally by `scripts/claude.sh` / `make cc`; see ADR `0023`) and re-run `/bootstrap`.
...
1. **GitHub login.** the runtime's question interface with the default from:
   ```bash
   gh api user --jq .login
   ```
2. **Project slug.** the runtime's question interface with the default from:
   ```bash
...
1. **GitHub repo — link to the one you created (Mode A never creates it).** Per ADR `0008` the repo is created by hand (empty) before bootstrap. Ensure `origin` points at it and the token can reach it:
   ...
   if ! gh repo view "$OWNER/$SLUG" >/dev/null 2>&1; then
     ...
     echo "  3) add GITHUB_PERSONAL_ACCESS_TOKEN=github_pat_... to .env (parsed literally by scripts/claude.sh; see ADR 0023) and re-run /bootstrap."
     exit 2
   fi
...
2. **Skeleton** — dispatch `devops` (`subagent_type: "devops"`) to:
   ...
   - Copy templates:
     ...
     - `templates/STUBS.md` -> `docs/STUBS.md`, then **strip the example row** and retitle for this project so it ships as an empty ledger (header + column definitions only), per docs/ai/rules/no-stubs.md — never leave the untouched template's example row.
...
3. **Stand up containers + Django** — dispatch `devops`:
   ...
   - Register the cross-cutting **`apps.common`** app and `drf_spectacular` in `INSTALLED_APPS` (in `base.py`):
     ...
     ```
   - Configure the project-wide **`REST_FRAMEWORK`** conventions in `base.py` (see `docs/ai/rules/serializers-permissions.md`). All five keys below are part of the scaffold contract; **keep** `DEFAULT_SCHEMA_CLASS` for drf-spectacular:
     ```python
     REST_FRAMEWORK = {
   ...
   - Configure **`drf-spectacular`** (see `docs/ai/rules/api-docs.md`):
...
4. **Initial commit + push; register checks only in GitHub mode** — dispatch `devops`:
   - Keep `templates/ai/` and the installer manifests: installed generator/schema checks depend on them. Do not delete templates or project files automatically. Check all scaffold destinations and unresolved substitution tokens.
   - Inspect the index and stage only the explicitly enumerated scaffold files; preserve unrelated staged/unstaged/untracked files.
   ...
   - Idempotent push (handles re-runs where the remote already exists):
     ...
     ```
   - **GitHub mode only:** trigger an initial CI run so GitHub registers `backend-ci` as a known
...
5. **Branch protection** — dispatch `ci-cd-engineer`:
   ...
   if [ "$CI_MODE" = local ]; then
     ...
     : "${VERIFIED_CHECK_CONTEXTS_JSON:?Inspect exact-head checks and set verified required_status_checks JSON first}"
     REQUIRED_STATUS_CHECKS="$VERIFIED_CHECK_CONTEXTS_JSON"
   else
     echo "CI mode missing or invalid" >&2; exit 2
   fi
   RULE_BODY=$(cat <<JSON
   {
   ...
   if [ $PROT_CODE -eq 0 ]; then
     echo "✓ Branch protection enabled on $OWNER/$SLUG for $CI_MODE mode"
   else
   ...
   5. In `github` mode, enable **Require status checks to pass before merging** and select only contexts observed on the exact candidate after a real run. In `local` mode, leave required hosted checks unset; preserve unrelated existing requirements during an update.
...
When the current request arguments contains `--dry-run`:
...
- Mode A: the first commit + `git push -u origin main` is the ONLY allowed direct-main push in this project (documented exception in `docs/ai/rules/git-operations.md`).
