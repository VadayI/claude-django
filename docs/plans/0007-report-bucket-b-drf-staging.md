...
**Крок 2 — зроблено (gunicorn-у-контейнері).** Відкрите питання #3 → вирішено: **gunicorn у контейнері** canonical (`docker-compose.staging.yml`). Додано: `gunicorn.conf.py`, health-route `/api/v1/health/` у `apps/common` (`views.py`/`urls.py`/`serializers.py` + тест), staging-env-ключі (`.env.example`), деплой-флоу з `check --deploy` + smoke (`docker-commands.md`, `guides_admin.md`), wiring у `bootstrap.md`.

**Звірка з паралельним PR #7 (закрито як дубль).** #7 був незалежною реалізацією тієї самої фічі з власним ADR. Рішення: лишити повнішу базу main (`gunicorn.conf.py`, `settings/test.py`), перейняти з #7 явно кращі/адитивні шматки — **`INSTALL_EXTRA=prod`** (тонкий dev-образ: gunicorn винесено з core deps в optional-групу `prod`; `backend.Dockerfile` отримав `ARG INSTALL_EXTRA=dev`; `docker-compose.staging.yml` передає `INSTALL_EXTRA=prod`), Makefile-таргет **`check-deploy`**, запис у `docs/lessons.md`. За «Simplicity first» **прибрано** `nginx.staging.conf.template` + `deploy/gunicorn.service.example` (та їх згадки в `bootstrap.md`/`docker-commands.md`) — systemd/nginx лишаються прозовою альтернативою в admin-гайді, не файлами-шаблонами.

**Відкрите питання #4 (ADR)** — переглянуто: ADR на DRF-конвенції не потрібен (вони були в scaffold до плану, ADR 0011 покриває config-базу), але на **staging-модель** ADR доречний — створено **ADR 0015 «Production-ready staging»** (звірений під фактичну реалізацію main, врятований і перероблений з PR #7).
