...
3. **Persist context** (delegate to `docs-writer`, or do it directly if trivial):
   ...
   - **Regenerate `docs/HANDOFF.md`** by running the `/handoff` logic (or delegating to `docs-writer`). This is mandatory: a wrap-up that leaves `HANDOFF.md` full of `{TODO}` placeholders has not finished. After regenerating, confirm `grep -c '{TODO}' docs/HANDOFF.md` is `0` (the carry-over `## Open questions` / `## Environment notes` sections may legitimately keep a `{TODO}`, but the state sections must be filled).
