...
- NOT already template-derived: if `docs/project-state/template-lineage.json` exists, OR `.claude/` + `CLAUDE.md` + `templates/` are all present → STOP and point to `/update-from-template` (the upgrade channel) or `/bootstrap` Mode B (an unfinished template scaffold).
...
4. **Dispatch `template-sync`** (`subagent_type: "template-sync"`) with `$UPSTREAM=/tmp/claude-django`, `MODE=adopt`, the layout survey, and the dry-run flag if present. Adopt mode is additive-only (see the agent's *Adopt mode* section): new files copied; existing files NEVER overwritten — upstream versions land as `<name>.adopt-proposed`; `templates/` is NOT copied wholesale; finishes by writing `docs/project-state/template-lineage.json` (`mode: adopt`) so all future upgrades go through `/update-from-template`.
