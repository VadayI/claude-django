...
1b. **Scenario detection.** Classify the project state into ONE of five:
   ...
   - `foreign-django` — a Django project exists (`manage.py` at the root or under `backend/`) BUT no template lineage: `.claude/memory/template-sync.json` absent AND (`CLAUDE.md` missing or without the claude-django `@`-import block) → recommend `/adopt` (additive attach via `template-sync`, ADR `0026`) — NOT `/bootstrap`, which assumes template scaffolding.
