...
0.5. **Runtime gate — run BEFORE the audit (hard STOP).** Read `.claude/memory/env-detect.json`. This file is the source of truth for platform/tooling and is written ONLY by the `SessionStart` hook of Claude Code CLI.
   ...
   - **If it EXISTS but `platform_supported == false`:** report `UNSUPPORTED_PLATFORM` and STOP, noting the detected `platform`. Since ADR `0022` made native Windows supported, this only occurs on a platform that is **not** Windows, macOS, Linux, or WSL2 — not expected on a normal dev machine; native Windows, WSL2, Linux, and macOS all pass. `wrong_runner_suspected` is retired (always `false`), so there is no longer a "you launched the Windows binary" sub-branch.
...
1. **Audit (read-only).** Dispatch the `devops` agent (`subagent_type: "devops"`) to run the read-only checks from `@.claude/rules/environment.md` for the requested scope(s). Instruct it explicitly:
   ...
   - **Platform audit** (schema v2+): read `.platform_supported` from `.claude/memory/env-detect.json`. Report ❌ `UNSUPPORTED_PLATFORM` only if `false` — which, since ADR `0022`, happens only on a platform that is none of Windows / macOS / Linux / WSL2 (native Windows, WSL2, Linux, and macOS all pass). If it ever fires, note the detected `platform` and STOP; `wrong_runner_suspected` is retired.
...
<!-- Last reviewed/updated: 2026-06-19 (ADR 0022: native Windows supported; platform gate STOPs only on a non-Windows/macOS/Linux/WSL2 platform; wrong_runner_suspected retired) -->
