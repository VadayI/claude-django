...
**5. GitHub access** — create the repo by hand + a fine-grained per-repo token (ADR `0008`). Create an **empty** repo at https://github.com/new (no README/.gitignore/license), then mint a **fine-grained token scoped to just that repo** (`/bootstrap` and `/doctor` print a ready-made template URL). Put the token in the project `.env` (alongside `CONTEXT7_API_KEY`) and launch Claude through the wrapper so it reaches the MCP servers and `gh` **without touching your shell rc** (ADR `0023`):

```bash
cp .env.example .env   # then edit .env: GITHUB_PERSONAL_ACCESS_TOKEN, CONTEXT7_API_KEY
bash scripts/claude.sh    # or:  make cc    (native PowerShell:  pwsh scripts/claude.ps1)
```

The wrapper copies your PAT into `GH_TOKEN` for `gh` and clears any stale `GITHUB_TOKEN` (a common cause of `gh` 401 on Windows). Minimal token permissions: **Contents** RW, **Metadata** RO (auto), **Pull requests** RW, **Workflows** RW, **Administration** RW (the last enables auto branch protection; omit it and protection becomes a manual UI step). A classic PAT works too but grants whole-account access.
