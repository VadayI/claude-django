...
def verify(root: Path, base: Path, context_path: Path) -> int:
    ...
    Returns: 0 when policy holds, 1 for an undocumented contract pin bump.
    ...
    Business rule: A changed checked-in CONTRACT_VERSION must include a
        docs/reviews or docs/decisions note in the exact candidate diff.
    """
    ...
    if ".env.example" in changed:
        old = contract_pin(base / ".env.example")
        new = contract_pin(root / ".env.example")
        if old != new and new is not None and not any(
            re.fullmatch(r"docs/(reviews|decisions)/.+\.md", name) for name in changed
        ):
