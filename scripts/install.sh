...
python "$CLONE/scripts/seed_preflight.py" --target "$TARGET" \
...
# Copy only preflighted inputs; never copy template memory or output language.
mkdir -p "$TARGET/.claude"
for entry in "$CLONE/.claude/"*; do
  [ "$(basename "$entry")" = memory ] && continue
  if [ "$(basename "$entry")" = rules ]; then
    mkdir -p "$TARGET/.claude/rules"
    for rule in "$entry/"*; do
      [ "$(basename "$rule")" = output-language.md ] && continue
      cp "$rule" "$TARGET/.claude/rules/"
    done
  else
    cp -r "$entry" "$TARGET/.claude/"
  fi
done
